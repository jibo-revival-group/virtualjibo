...
## Testing notes:
here's a skill console one-liner to reset the KB for this skill:

```jibo.kb.createModel('/word-of-the-day').loadRoot().then((root)=>{root.data = {}; return root;}).then((root)=>root.save()).then(()=>jibo.kb.createModel('/word-of-the-day').createModel('played-words').loadRoot().then((root)=>{root.data = {}; return root;}).then((root)=>root.save()).then(()=>{console.warn('WotD KB Reset!');}));```
