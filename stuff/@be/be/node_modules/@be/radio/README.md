...
**Since jibo-sync and jibo-run take so long, for OSX first run `yarn sync` and then use `jibo run --nosync` when using the CLI.**
...
|`yarn sync` | Sync file to the robot (uses `rsync`, **OSX only**)</br> **Tip**: Use `jibo run --nosync` or `jibo debug --nosync` after to launch skills |
